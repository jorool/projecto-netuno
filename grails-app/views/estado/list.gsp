			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="abreviacao" title="${message(code: 'estado.abreviacao.label', default: 'Abreviacao')}" />
					
						<g:sortableColumn property="codigoIBGE" title="${message(code: 'estado.codigoIBGE.label', default: 'Codigo IBGE')}" />
				...
				<g:each in="${estadoInstanceList}" status="i" var="estadoInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td>${fieldValue(bean: estadoInstance, field: "abreviacao")}</td>
					
						<td>${fieldValue(bean: estadoInstance, field: "codigoIBGE")}</td>
